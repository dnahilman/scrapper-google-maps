...
func main() {
	...
	if strings.EqualFold(os.Getenv("SCRAPER"), "noop") {
		...
		pw := scraper.NewPlaywrightExecutor(cfg, logger.L())
		// Wire per-place live event callback — fires after each successful ScrapePlace.
		agentClient := workeragent.NewClient(cfg.MasterURL, cfg.MasterToken)
		pw.OnPlaceScraped = func(ctx context.Context, placeID, title string, index, total int) {
			_ = agentClient.PublishPlaceScraped(ctx, &workeragent.PlaceScrapedEventPayload{
				PlaceID: placeID,
				Title:   title,
				Index:   index,
				Total:   total,
