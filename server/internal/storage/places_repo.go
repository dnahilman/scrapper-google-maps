...
func (r *PlacesRepo) ListByJob(ctx context.Context, jobID uuid.UUID, limit, offset int) ([]domain.Place, error) {
	var out []domain.Place
	err := r.db.WithContext(ctx).
		Where("task_id IN (SELECT id FROM tasks WHERE job_id = ?)", jobID).
...
func (r *PlacesRepo) CountByJob(ctx context.Context, jobID uuid.UUID) (int64, error) {
	var n int64
...
func (r *PlacesRepo) ListAllByJob(ctx context.Context, jobID uuid.UUID) ([]domain.Place, error) {
	var out []domain.Place
	err := r.db.WithContext(ctx).
		Where("task_id IN (SELECT id FROM tasks WHERE job_id = ?)", jobID).
		Order("scraped_at DESC").
