...
type PlaywrightExecutor struct {
	...
	log             *zerolog.Logger
	OnPlaceScraped  func(ctx context.Context, placeID, title string, index, total int)
}
...
func (e *PlaywrightExecutor) Execute(ctx context.Context, task *queue.ClaimedTask) ([]domain.PlacePayload, error) {
	...
	for i, u := range urls {
		...
		out = append(out, *p)
		if e.OnPlaceScraped != nil {
			e.OnPlaceScraped(ctx, p.PlaceID, p.Title, len(out), len(urls))
		}
		if (i+1)%5 == 0 {
