...
func registerInternalRoutes(g *gin.RouterGroup, d *Deps) {
	...
	g.POST("/events/place-scraped", placeScrapedEvent(d))
}
		...
		for _, p := range req.Places {
			...
			if err := d.Places.Upsert(c.Request.Context(), place); err != nil {
				...
				if errors.As(err, &pgErr) && pgErr.Code == "23503" {
					// Task was deleted (job cancelled/deleted) while worker was running.
					// Return 410 so the worker knows to abandon silently, not retry.
...
type PlaceScrapedEventRequest struct {
...
func placeScrapedEvent(d *Deps) gin.HandlerFunc {
	return func(c *gin.Context) {
		var req PlaceScrapedEventRequest
		if err := c.ShouldBindJSON(&req); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
		if d.Hub != nil {
			d.Hub.Broadcast(logstream.EventPlaceScraped, req)
		}
		c.Status(http.StatusNoContent)
	}
}
